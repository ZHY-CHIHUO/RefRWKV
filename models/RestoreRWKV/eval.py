    ...
    with torch.no_grad():
        for batch_idx, (lr1, ref, lr2, hr_gt) in enumerate(tqdm(test_loader, desc='Testing')):
            ...
            if pred.shape[-2:] != hr_gt.shape[-2:]:
                pred = F.interpolate(pred, size=hr_gt.shape[-2:], mode='bicubic', align_corners=False)
