...
class RefSRWKV(nn.Module):
    def __init__(
        ...
        self.ref_down2 = Downsample(dim)
        self.ref_down3 = Downsample(dim * 2)
        self.ref_down4 = Downsample(dim * 4)

        # ---- 各层级融合卷积 (1×1) ----
        self.fuse1 = nn.Conv2d(dim * 2, dim, 1)
        self.fuse2 = nn.Conv2d(dim * 4, dim * 2, 1)
        self.fuse3 = nn.Conv2d(dim * 8, dim * 4, 1)
        self.fuse4 = nn.Conv2d(dim * 16, dim * 8, 1)
        ...
        # ---- 解码器 ----
        self.up4_3 = Upsample(dim * 8)  # 通道: 8*dim → 4*dim
        ...
        self.up3_2 = Upsample(dim * 4)  # 通道: 4*dim → 2*dim
        ...
        self.up2_1 = Upsample(dim * 2)  # 通道: 2*dim → dim
    ...
    def forward(self, lr1, hr1, label=None):
        ...
        # 3. 编码器 + 参考注入
        # 编码器
        e1 = self.encoder_level1(self.fuse1(torch.cat([fea, ref_1], dim=1)))
        e2 = self.encoder_level2(
            self.fuse2(torch.cat([self.down1_2(e1), ref_2], dim=1))
        )
        e3 = self.encoder_level3(
            self.fuse3(torch.cat([self.down2_3(e2), ref_3], dim=1))
        )
        l = self.latent(self.fuse4(torch.cat([self.down3_4(e3), ref_4], dim=1)))
        ...
        # 6. 高分辨率参考引导 (直接使用原始HR)
        refine_input = torch.cat([out, hr1], dim=1)
        residual = self.hr_refine(refine_input)
        out = out + residual
...
if __name__ == "__main__":
    ...
    model = RefSRWKV(inp_channels=3, out_channels=3, dim=48, scale=10).to(device)
