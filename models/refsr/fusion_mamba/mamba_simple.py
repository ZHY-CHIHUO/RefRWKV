...
try:
    ...
    from mamba_ssm.ops.selective_scan_interface import causal_conv1d_fn as _causal_conv1d_fn
    try:
...
if mamba_inner_fn_no_out_proj is None and selective_scan_fn is not None:
    def mamba_inner_fn_no_out_proj(
        xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, A,
        B=None, C=None, D=None, delta_bias=None, delta_softplus=True,
    ):
        """Compatibility implementation for newer mamba_ssm releases."""
        x, z = xz.chunk(2, dim=1)
        if _causal_conv1d_fn is not None:
            x = _causal_conv1d_fn(
                x, rearrange(conv1d_weight, "d 1 w -> d w"), conv1d_bias,
                activation="silu",
            )
        else:
            x = F.silu(F.conv1d(x, conv1d_weight, conv1d_bias, padding=conv1d_weight.shape[-1] - 1)[..., :xz.shape[-1]])
        length = x.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1]
        x_dbl = F.linear(rearrange(x, "b d l -> (b l) d"), x_proj_weight)
        delta = delta_proj_weight @ x_dbl[:, :delta_rank].t()
        delta = rearrange(delta, "d (b l) -> b d l", l=length)
        if B is None:
            B = rearrange(x_dbl[:, delta_rank:delta_rank + d_state], "(b l) d -> b d l", l=length).contiguous()
        if C is None:
            C = rearrange(x_dbl[:, -d_state:], "(b l) d -> b d l", l=length).contiguous()
        y = selective_scan_fn(
            x, delta, A, B, C, D, z=z,
