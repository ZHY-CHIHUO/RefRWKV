...
except Exception:  # optional; only used for FLOP reports
...
class SS2D(nn.Module):
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        if forward_type == "v2" and "selective_scan_cuda_core" not in globals():
            forward_type = "v01"
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        if self.forward_core is None and "v01" in FORWARD_TYPES:
            self.forward_core = FORWARD_TYPES["v01"]
        k_group = 4
...
class VSSM(nn.Module):
    ...
    def flops(self, shape=(3, 224, 224)):
        ...
        params = parameter_count(model)[""] if parameter_count is not None else 0
