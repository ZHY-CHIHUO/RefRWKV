#!/usr/bin/env python3
"""Train a direct registry-based reference-super-resolution model."""
...
def run(
    ...
    *,
    ...
    if model_name == "stf_mamba":
        from engines.refsr.stf_mamba_trainer import STFMambaTrainer

        trainer_cls = STFMambaTrainer
    elif model_name in {"rdm_refsr", "rdm_mhf"}:
        trainer_cls = RDMRefSRTrainer
    else:
        trainer_cls = RefSRTrainer
    ...
    logger.info(
        "RefSR 训练: model=%s dataset=%s scale=x%s reference_mode=%s",
        config["model"]["name"],
        config["dataset"]["id"],
        config["data"]["scale"],
        config["data"].get("reference_mode"),
    )
