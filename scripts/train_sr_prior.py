...
def check_resume_compatible(ckpt_path: str, lit_model):
    ...
    if mismatched:
        detail = "; ".join(f"{k}: ckpt{c} vs 现模型{r}" for k, c, r in mismatched[:3])
        return False, f"{len(mismatched)} 个参数形状不匹配: {detail}"
...
def load_weights_filtered(lit_model, ckpt_path: str):
    ...
    for k, v in sd.items():
        ...
        if full_key is None:
            continue
        if tuple(v.shape) == tuple(ref[full_key].shape):
            matched[full_key] = v
        else:
            skipped.append(k)
...
def main():
    ...
    if "output" not in cfg:
        logger.warning(
            "yaml 未配置 output 段，使用默认路径: checkpoint=%s, log=%s, experiment=%s",
            ckpt_dir,
            log_dir,
            exp_name,
        )
    ...
    trainer = pl.Trainer(
        ...
        gradient_clip_algorithm=tc.get("gradient_clip_algorithm", "norm"),
        callbacks=callbacks,
    ...
    logger.info(
        ...
        tc.get("gradient_clip_algorithm", "norm"),
    )
    logger.info(
        "  EMA: %s",
        (
    ...
    if args.load_weights:
        logger.info("  权重来源: 热启动 %s", args.load_weights)
    else:
