...
CONFIG="configs/runs/rdm_refsr/pancollection_wv3_x4.yaml"
OVERRIDES=(
  data.max_samples_train=128
...
case "${1:-}" in
  --help|-h)
    cat <<'EOF'
用法:
  rdm_refsr_pancollection_wv3_quick.sh [--run|--print] [训练参数]

默认使用完整 RDMRefSR 网络，仅限制 WV3 样本数和 300 steps。
