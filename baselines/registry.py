"""Small registry for HR/LR single-image SR baselines.

Every baseline adapter builds a model from the same run configuration.  The
...
_ADAPTERS: dict[str, BaselineAdapter] = {}
...
def register_adapter(adapter: BaselineAdapter) -> BaselineAdapter:
    ...
    if not name:
        raise ValueError("baseline adapter name must be non-empty")
    if name in _ADAPTERS:
        raise ValueError(f"baseline adapter already registered: {name}")
    adapter.name = name
...
def get_adapter(name: str) -> BaselineAdapter:
    ...
    try:
        ...
        raise KeyError(f"unknown baseline model {name!r}; available: {available}") from exc
