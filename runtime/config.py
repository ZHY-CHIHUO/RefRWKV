...
DIRECT_REFSR_MODEL_NAMES = frozenset({"refsrwkv", "rdm_refsr", "rdm_pan", "rdm_stf", "rdm_mhf", "fusion_mamba", "stf_mamba", "ttsr", "masa_sr", "datsr"})
...
def normalize_reference_mode(value: Any) -> str:
    ...
    ``paired`` consumes an on-disk ``Ref/`` image.  ``lr_up`` is the legacy
    SISR fallback used by direct RefSR models on HR/LR-only datasets. ``none``
    disables the reference branch entirely for a true SISR run.
    """
    normalized = str(value).strip().lower()
    try:
        return _REFERENCE_MODE_ALIASES[normalized]
    except KeyError as exc:
        options = ", ".join(sorted({"none", "paired", "lr_up"}))
...
def validate_refsr_reference_contract(config: Mapping[str, Any]) -> None:
    ...
    use_reference = model.get("use_reference", True)
    if not isinstance(use_reference, bool):
        raise ValueError("model.use_reference 必须是 bool")
    if not use_reference and mode != "none":
        raise ValueError(
            "model.use_reference=false 必须配合 data.reference_mode=none；"
            "不会再为单 SR 路径生成 bicubic 伪参考。"
        )
    if use_reference and mode == "none":
        raise ValueError(
            "model.use_reference=true 不能配合 data.reference_mode=none；"
            "请使用 paired 或 lr_up。"
        )
    if model_name in PAIRED_REFERENCE_MODEL_NAMES and not use_reference:
        raise ValueError(
            f"model.name={model.get('name', model_name)} 必须启用真实 Ref 分支。"
        )
